...
void LCD_DisplayStringLineInRect(uint16_t Xpos, uint16_t Ypos, uint16_t Width, uint16_t Height, uint8_t *ptr)
{
	uint8_t *s_ptr;
	uint16_t refcolumn = 0;
	uint16_t length_of_label = 0;
	uint8_t Line;
	s_ptr = ptr; 
/*find the length of string*/
	while(*ptr != 0)
	{
		length_of_label += LCD_Currentfonts->Width;
    ptr++;
	}
	ptr = s_ptr; 
	if (LCD_Currentfonts->Height >  Height )
	{
		Line = Ypos;
	}
	else 
		Line = Ypos + (Height - LCD_Currentfonts->Height)/2;
	if (LCD_Currentfonts->Width >  Width )
	{	
		refcolumn = Xpos;
	}
	else refcolumn = Xpos + (Width - length_of_label)/2;
  /* Send the string character by character on lCD */
  while ((*ptr != 0) & (((refcolumn + 1) & 0xFFFF) <= (LCD_PIXEL_WIDTH - 1)))
  {
    /* Display one character on LCD */
    LCD_DisplayCharTransparent(Line, refcolumn,*ptr);
    /* Decrement the column position by 16 */
    refcolumn += LCD_Currentfonts->Width;
    /* Point on the next character */
    ptr++;
