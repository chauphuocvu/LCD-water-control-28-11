...
static void GL_DrawRectangle(uint16_t maxX, uint16_t minX, uint16_t maxY, uint16_t minY)
{
  ...
  GL_DrawLine(minX, minY, maxY - minY, GL_Vertical);
  GL_DrawLine(maxX -1, minY, maxY - minY, GL_Vertical);
}
...
//		GL_DrawLine(minX, maxY, maxX - minX, GL_Horizontal);
//		GL_DrawLine(minX, minY, maxY - minY, GL_Vertical);
//		GL_DrawLine(maxX, minY, maxY - minY +1, GL_Vertical);
//  }
...
//	GL_SetDisplayWindow(minX, minY, maxX - minX +1, maxY - minY +1);
  GL_DrawBMP(ptrBitmap);
  GL_SetDisplayWindow(LCD_Height - 1, LCD_Width - 1, LCD_Height, LCD_Width);
//	GL_SetDisplayWindow(0,0,LCD_Width-1, LCD_Height-1);
}
...
//				AddPageControlObj( (uint16_t)PosX,
                           (uint8_t)PosY,
...
//                             (uint16_t)(objCoordinates.MaxX-((MAX_BUTTON_LABEL_LENGTH-1)*FONT_LENGTH)), 
			GL_DrawFilledRectangle((uint16_t)(objCoordinates.MinX+((MAX_BUTTON_LABEL_LENGTH-1)*FONT_LENGTH)), 
                             (uint16_t)(objCoordinates.MinX), 
...
//                             (uint16_t)(objCoordinates.MaxX-((LabelLength+1)*FONT_LENGTH)), 
			GL_DrawFilledRectangle((uint16_t)(objCoordinates.MinX+((LabelLength+1)*FONT_LENGTH)), 
                             (uint16_t)(objCoordinates.MinX), 
                             (uint8_t)(objCoordinates.MaxY), 
                             (uint8_t)(objCoordinates.MinY), GL_Blue);
#endif
...
//    for ( ; n < LabelLength && (objCoordinates.MaxX - ((n + 1)*BUTTON_SLICE_LENGTH)) > BUTTON_SLICE_LENGTH + 1; n++)
		for ( ; n < LabelLength && (LCD_Width - (objCoordinates.MinX + ((n + 1)*BUTTON_SLICE_LENGTH))) > BUTTON_SLICE_LENGTH + 1; n++)
    {
...
//    for ( ; n < LabelLength - 1 && (objCoordinates.MaxX - ((n + 1)*BUTTON_SLICE_LENGTH)) > BUTTON_SLICE_LENGTH + 1; n++)
		for ( ; n < LabelLength -1 && (LCD_Width - (objCoordinates.MinX + ((n + 1)*BUTTON_SLICE_LENGTH))) > BUTTON_SLICE_LENGTH + 1; n++)
    {
//      GL_DrawButtonBMP( (uint16_t)(objCoordinates.MaxX - (n*BUTTON_SLICE_LENGTH)),
//                        (uint16_t)(objCoordinates.MaxX - ((n + 1)*BUTTON_SLICE_LENGTH)),
			GL_DrawButtonBMP( (uint16_t)(objCoordinates.MinX + ((n+1)*BUTTON_SLICE_LENGTH)),
...
//  if ( (objCoordinates.MaxX - ((n + 1)*BUTTON_SLICE_LENGTH)) > 0 )
	if ( (LCD_Width - (objCoordinates.MinX + ((n + 1)*BUTTON_SLICE_LENGTH))) > 0 )
  {
//    GL_DrawButtonBMP( (uint16_t)(objCoordinates.MaxX - (n*BUTTON_SLICE_LENGTH)),
//                      (uint16_t)(objCoordinates.MaxX - ((n + 1)*BUTTON_SLICE_LENGTH)),
		GL_DrawButtonBMP( (uint16_t)(objCoordinates.MinX + ((n + 1)*BUTTON_SLICE_LENGTH)),
...
//                            (uint16_t)(objCoordinates.MaxX) - ((btn_length - (p_strlen((pThis->label_1))+1)*FONT_LENGTH) / 2) - 3,
														(uint16_t)(objCoordinates.MaxX) - maxLabelLength*FONT_LENGTH,
...
//                           (uint16_t)(objCoordinates.MaxX) - (RBUTTON_OPT_SIZE + 2),
													 (uint16_t)(objCoordinates.MaxX)+5,
                           (uint8_t*)pThis->label,
                           GL_TRUE);
...
//														 (uint16_t)(objCoordinates.MinX) + (CHECKBOX_SIZE + 1) + n*FONT_LENGTH,
//                             (uint8_t*)' ',
//                             GL_FALSE);
//  }
  GL_SetTextColor(GL_Black);
...
//                           (uint16_t)(objCoordinates.MaxX) - (CHECKBOX_SIZE + 1),
													 (uint16_t)(objCoordinates.MinX) + (CHECKBOX_SIZE + 5),
                           (uint8_t*)pThis->label,
                           GL_TRUE);
  GL_SetFont(GL_FONT_BIG);
...
#ifndef USE_2D_OBJECTS
    GL_DrawButtonBMP( maxX, maxX - SLIDEBAR_PIECE_LENGTH, maxY, minY, 
//										ptrBmpLeft );
										ptrBmpRight );
...
//                               maxX - ((btn_length - label_length*FONT_LENGTH) / 2) - 3,
															  maxX - ((label_length+1)*FONT_LENGTH),
                               (uint8_t*)pThis->label,
                               GL_TRUE);
    }
    else if (pThis->Direction == GL_LEFT_VERTICAL)
