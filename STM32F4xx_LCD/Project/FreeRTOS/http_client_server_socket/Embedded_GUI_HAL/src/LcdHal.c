...
{
//	LCD_DrawCharTransparent(Xpos, Ypos, (uint16_t*)c);
...
//    {
//      if((((c[line_index] & ((0x80 << ((GL_FontWidth / 12 ) * 8 ) ) >> pixel_index)) == 0x00) &&(GL_FontWidth <= 12))||
//        (((c[line_index] & (0x1 << pixel_index)) == 0x00)&&(GL_FontWidth > 12 )))
//      {			
//      }
//      else
//      {
//        SSD1963_PutPixel(Xpos + pixel_index,Ypos + line_index);
//      }
//    }
//  }
	
  uint32_t line_index = 0, pixel_index = 0;
	// old code use 240x320 LCD, so Xaddress: uint8_t, Xaddress: uint16_t
  uint16_t Xaddress = 0;
  uint16_t Yaddress = 0;
...
{
	uint16_t tmpColor = GL_TextColor;
	GL_SetTextColor(Color);
	SSD1963_PutPixel(Xpos, Ypos);
	GL_SetTextColor(tmpColor);
...
//  LCD_SetCursor(Xpos, Ypos);
	tmpColor = SSD1963_GetPixel(Xpos, Ypos);
//  if ( pLcdHwParam.LCD_Connection_Mode == GL_SPI)
//  {
...
//  tmpRed = (tmpColor & GL_Blue) << 11;
//  tmpBlue = (tmpColor & GL_Red) >> 11;
//  tmpColor &= ~(GL_Red | GL_Blue);
...
{
//	LCD_DrawChar(Ypos, Xpos, (uint16_t*)c);
...
//    LCD_SetCursor(Xaddress, Ypos);
		LCD_SetArea(Xpos, Ypos, Xpos + GL_FontWidth -1, Ypos + GL_FontHeight -1);
  }
  else if ((LCD_Direction == _90_degree) || (LCD_Direction == _270_degree))
  {
    Yaddress = Ypos;
//    LCD_SetCursor(Xpos, Yaddress);
		LCD_SetArea(Xpos, Ypos, Xpos + GL_FontHeight -1, Ypos + GL_FontWidth -1);
  }
...
//			if((((c[index] & ((0x80 << ((GL_FontWidth / 12 ) * 8 ) ) >> counter)) == 0x00) &&(GL_FontWidth <= 12))||
//        (((c[index] & (0x1 << counter)) == 0x00)&&(GL_FontWidth > 12 )))
      {
...
//	LCD_DisplayChar(Line, Column, Ascii);
  ...
  {
    case GL_FONT_BIG:
      LCD_WriteChar(Column, Line ,&GL_Font16x24.table[Ascii * GL_FontHeight] );
      break;

    case GL_FONT_SMALL:
      LCD_WriteChar(Column, Line ,&GL_Font8x12_bold.table[Ascii * GL_FontHeight] );
      break;
...
{
//	LCD_DisplayAdjStringLine(Line, Column, (uint8_t*)ptr, LCD_TRUE);
  uint32_t counter = 0;
...
//    refcolumn = Column;
  ...
  {
    max_char_line = (LCD_Width / GL_FontWidth);
    refcolumn = Column - GL_FontWidth;
//		refcolumn = LCD_Width - Column;
//    line = LCD_Width - Line;
  }
  else if (LCD_Direction == _270_degree)
  {
...
void LCD_DrawColorBMP(uint8_t* ptrBitmap, uint16_t Xpos_Init, uint16_t Ypos_Init, uint16_t Height, uint16_t Width)
{
  ...
  BmpBuffer32BitRead(&uDataAddr, ptrBitmap + 10);  
	...
	GL_SetDisplayWindow(Xpos_Init, Ypos_Init,(Xpos_Init + Width - 1),(Ypos_Init + Height - 1));
