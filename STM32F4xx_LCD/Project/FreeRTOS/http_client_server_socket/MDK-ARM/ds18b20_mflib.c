...
{
	GPIO_ResetBits(TXRX_PORT,GPIO_Pin_0);
	DelayMicro(600);
	GPIO_SetBits(TXRX_PORT,GPIO_Pin_0);
	DelayMicro(100);
	if (GPIO_ReadInputDataBit(TXRX_PORT, TXRX_PIN) == 0) {
		DelayMicro(500);
		return 1;
	} else {
		DelayMicro(500);
...
void ds18b20_send_function_cmd(uint8_t cmd)
{
	...
	if (cmd == CONVERT_T_CMD) {
		while(GPIO_ReadInputDataBit(TXRX_PORT, TXRX_PIN) == 0) {
